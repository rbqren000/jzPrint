package com.org.jzprinter.XXPermissions;
...
public final class PermissionInterceptor implements OnPermissionInterceptor {
    ...
    public PermissionInterceptor(String permissionDescription) {
        if(!TextUtils.isEmpty(permissionDescription)){
            String[] split = permissionDescription.split("\n");
            if(split.length>=2){
                mPermissionTitle = split[0];
                mPermissionDescription = split[1];
            }
        }
    }

    @Override
    public void launchPermissionRequest(@NonNull Activity activity, @NonNull List<String> allPermissions, OnPermissionCallback callback) {
        mRequestFlag = true;
        ...
        if (TextUtils.isEmpty(mPermissionTitle)) {
            mPermissionTitle = activity.getString(R.string.common_permission_description_title);
        }
        ...
        if (showPopupWindow) {
            ...
            new AlertDialog.Builder(activity, R.style.mAlertDialog)
...
//                Toaster.show();
    ...
    private void showPopupWindow(Activity activity, ViewGroup decorView,String title, String message) {
        ...
        TextView titleView = mPermissionPopup.getContentView().findViewById(R.id.tv_permission_title);
