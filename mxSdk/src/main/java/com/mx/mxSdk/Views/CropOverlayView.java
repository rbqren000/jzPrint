...
import com.mx.mxSdk.OpencvUtils.Cropper;
import com.mx.mxSdk.OpencvUtils.Scanner;
...
/**
 * Created by rbq on 8/2/17.
 */

public class CropOverlayView extends AppCompatImageView {

    private static final String TAG = "CropImageView";

    private static final float TOUCH_POINT_CATCH_DISTANCE = 25; //dp，触摸点捕捉到锚点的最小距离（从15增加到25）
    private static final float POINT_RADIUS = 12; // dp，锚点绘制半径（从10增加到12）
    ...
    private static final int DEFAULT_POINT_SELECTED_COLOR = 0xFFFF4081; // 选中状态的颜色

    private Paint mPointPaint;
    private Paint mPointFillPaint;
    private Paint mPointSelectedPaint; // 选中状态的锚点填充画笔
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CropOverlayView);
        mMaskAlpha = Math.min(Math.max(0, ta.getInt(R.styleable.CropOverlayView_civMaskAlpha, DEFAULT_MASK_ALPHA)), 255);
        mShowGuideLine = ta.getBoolean(R.styleable.CropOverlayView_civShowGuideLine, true);
        mLineColor = ta.getColor(R.styleable.CropOverlayView_civLineColor, DEFAULT_LINE_COLOR);
        mLineWidth = ta.getDimension(R.styleable.CropOverlayView_civLineWidth, dp2px(DEFAULT_LINE_WIDTH));
        mMagnifierCrossColor = ta.getColor(R.styleable.CropOverlayView_civMagnifierCrossColor, DEFAULT_MAGNIFIER_CROSS_COLOR);
        mShowMagnifier = ta.getBoolean(R.styleable.CropOverlayView_civShowMagnifier, true);
        mGuideLineWidth = ta.getDimension(R.styleable.CropOverlayView_civGuideLineWidth, dp2px(DEFAULT_GUIDE_LINE_WIDTH));
        mGuideLineColor = ta.getColor(R.styleable.CropOverlayView_civGuideLineColor, DEFAULT_GUIDE_LINE_COLOR);
        mPointFillColor = ta.getColor(R.styleable.CropOverlayView_civPointFillColor, DEFAULT_POINT_FILL_COLOR);
        mPointFillAlpha = Math.min(Math.max(0, ta.getInt(R.styleable.CropOverlayView_civPointFillAlpha, DEFAULT_POINT_FILL_ALPHA)), 255);
        ta.recycle();
    }
    ...
    private void initPaints() {
        ...
        mPointSelectedPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPointSelectedPaint.setColor(mPointSelectedColor);
        mPointSelectedPaint.setStyle(Paint.Style.FILL);
        mPointSelectedPaint.setAlpha(mPointFillAlpha);
    ...
    protected void onDrawPoints(Canvas canvas) {
        ...
        for (Point point : mCropPoints) {
            // 如果是当前拖动的锚点，使用选中状态的颜色
            if (point == mDraggingPoint) {
                canvas.drawCircle(getViewPointX(point), getViewPointY(point), dp2px(POINT_RADIUS), mPointSelectedPaint);
            } else {
    ...
    private void toImagePointSize(Point dragPoint, MotionEvent event) {
        ...
        // 放宽边界限制，允许锚点稍微超出图片边界（10dp范围）
        float boundaryMargin = dp2px(10);
        float x = Math.min(Math.max(event.getX(), mActLeft - boundaryMargin), mActLeft + mActWidth + boundaryMargin);
        float y = Math.min(Math.max(event.getY(), mActTop - boundaryMargin), mActTop + mActHeight + boundaryMargin);
        
        // 使用double类型提高精度
        dragPoint.x = (x - mActLeft) / mScaleX;
        dragPoint.y = (y - mActTop) / mScaleY;
        
        // 确保坐标在合理范围内
        dragPoint.x = Math.max(0, Math.min(dragPoint.x, getDrawable().getIntrinsicWidth()));
        dragPoint.y = Math.max(0, Math.min(dragPoint.y, getDrawable().getIntrinsicHeight()));
    }
