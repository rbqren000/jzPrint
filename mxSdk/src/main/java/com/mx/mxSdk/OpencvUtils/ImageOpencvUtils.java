...
public class ImageOpencvUtils {
	...
	public static int[] cutRectHelp(Point[] rectPoint) {
		...
		for (int i = 1; i < rectPoint.length; i++) {
			minX = Math.min(rectPoint[i].x, minX);
			maxX = Math.max(rectPoint[i].x, maxX);
			minY = Math.min(rectPoint[i].y, minY);
			maxY = Math.max(rectPoint[i].y, maxY);
		}
		return new int[]{ (int) Math.abs(minX), (int) Math.abs(minY), (int) Math.abs(maxX - minX),
