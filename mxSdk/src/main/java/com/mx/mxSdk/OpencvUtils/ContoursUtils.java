    ...
    public static List<MatOfPoint> findContours(Mat cannyMat) {
        ...
        if (contours.size() <= 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                MatOfPoint[] ary = (MatOfPoint[]) contours.toArray();

                for(int i = 0;  i < ary.length - 1;  i++){
                    for(int j = i + 1;  j < ary.length;  j++){

                        MatOfPoint current = ary[i];

                        MatOfPoint2f mat1 = new MatOfPoint2f(ary[i].toArray());
                        RotatedRect rect1 = Imgproc.minAreaRect(mat1);
                        Rect r1 = rect1.boundingRect();

                        MatOfPoint2f mat2 = new MatOfPoint2f(ary[j].toArray());
                        RotatedRect rect2 = Imgproc.minAreaRect(mat2);
                        Rect r2 = rect2.boundingRect();

                        if(r1.area() > r2.area()){
                            MatOfPoint tmp = ary[j];
                            ary[j] = current;
                            ary[i] = tmp;
                        }
                    }
                }
                contours = Arrays.asList(ary);
            }
            return contours;
        }
    }
